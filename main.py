...
def render_deep_model_logic(tool_name, tool_meta_name, context):
    ...
    # 如果任务已经完成（查看历史记录时），直接显示图表，不显示控制台
    ...
    with st.container(border=True):
        ...
        if current_status == "untrained":
            ...
            if not st.session_state.get(f"trained_{tool_name}"):
                ...
                if st.button("⚡ 开始全量数据训练", key=f"btn_train_{tool_name}", use_container_width=True):
                    # 模拟进度条
        ...
        # =================================================
            ...
            # 状态机：idle (选择模式) -> finetuning (微调中) -> done (完成决策)
            ...
            if mode_key not in st.session_state:
                ...
                # 按钮 B: 不要增量微调
                if b2.button("⏩ 不要增量微调 (直接预测)", key=f"ft_no_{tool_name}", use_container_width=True):
            ...
            # 3. 模式 B: 执行增量微调
                ...
                return True  # 显示图表 (在按钮上方或下方显示由 tool_xxx.view 决定，通常是下方)
...
def render_analysis_page():
    ...
    # 2. 显示历史消息 (只显示非工具执行过程的文本，保持页面清爽)
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]):
            if not msg.get("is_tool_process"):  # 过滤掉中间过程，只显示对话
    ...
    # 4. 核心：任务执行大框
    # 修改点：只要有 current_workflow 就始终渲染，不再依赖最后一条消息的角色
    if st.session_state.get("current_workflow") or (
        ...
        if workflow:
                ...
                for i, tool_id in enumerate(workflow):
                    ...
                    with st.expander(step_title, expanded=expander_open):
                        try:
                            ...
                            # 1. 运行工具 (仅在当前活跃步骤且未完成时运行耗时逻辑)
                            # 这样完成后再点开看，就不会重新 sleep 等待了，非常快
                            if is_current_active:
                                # [New] 增加通用延时，让每个步骤运行约 5 秒
                                delay_bar = st.progress(0, text=f"⏳ {meta['name']} 正在执行...")
                                for i in range(100):
                                    time.sleep(0.05)  # 0.05s * 100 = 5s
                                    delay_bar.progress(i + 1)
                                delay_bar.empty()
            ...
            with st.chat_message("assistant"):
                if is_finished:
                    # 场景 B: 任务已完成 -> 切换为 st.expander
                    # 不设置 expanded=True，默认就是折叠的。
                    # 当用户手动点开时，Streamlit 会记住状态，不会回弹。
                    with st.expander("✅ 所有步骤执行完毕 (点击查看详情/操作历史)"):
                        render_tool_steps()
                else:
                    # 场景 A: 任务运行中 -> 使用 st.status
                    # expanded=True 保证运行过程一直可见
    ...
    if st.session_state.get("workflow_finished", False):
        # 防止重复追加
        last_msg = st.session_state.messages[-1]
        # 只要最后一条不是 assistant 发的总结文本，就追加
        # (这里简单通过判断 content 是否包含 "核心结论" 来避免重复，或者通过 role 判断)
